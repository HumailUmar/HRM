...
export class ZKTecoAdapter implements IBiometricAdapter {
  private connected = false;
  private config: BiometricDeviceConfig | null = null;

  private async request<T>(endpoint: string, body: any): Promise<T> {
    const response = await fetchWithRetry(`/api/zkteco/${endpoint}`, {
      method: 'POST',
  ...
  async connect(config: BiometricDeviceConfig): Promise<boolean> {
    this.config = config;
    this.connected = true;
    logger.info(`🔵 ZKTeco Device connected: ${config.host}:${config.port}`);
    return true;
  }
  ...
  async disconnect(): Promise<void> {
    ...
    this.config = null;
    logger.info('🔴 ZKTeco Device disconnected');
  }
  ...
  async fetchPunches(startDate?: string, endDate?: string): Promise<BiometricPunchRecord[]> {
    ...
    if (!result || typeof result !== 'object' || !Array.isArray(result.records)) {
      throw new Error(`Invalid response from ZKTeco device: expected records array, got ${JSON.stringify(result)}`);
    }
