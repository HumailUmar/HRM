...
export class HikvisionAdapter implements IBiometricAdapter {
  ...
  async connect(config: BiometricDeviceConfig): Promise<boolean> {
    this.config = config;
    this.connected = true;
    logger.info(`🔵 Hikvision Device connected: ${config.host}:${config.port}`);
    return true;
  }
  ...
  async disconnect(): Promise<void> {
    ...
    this.config = null;
    logger.info('🔴 Hikvision Device disconnected');
  }
  ...
  async fetchPunches(startDate?: string, endDate?: string): Promise<BiometricPunchRecord[]> {
    ...
    if (!result || typeof result !== 'object' || !Array.isArray(result.records)) {
      throw new Error(`Invalid response from Hikvision device: expected records array, got ${JSON.stringify(result)}`);
    }
  ...
  async fetchUsers(): Promise<{ id: string; name: string; employeeId?: string }[]> {
    ...
    try {
      ...
      logger.error('Failed to fetch Hikvision users:', error);
    }
    return [];
  }
  ...
  async syncUsers(employees: any[]): Promise<{ synced: number; failed: number }> {
    ...
    try {
      ...
      logger.error('Failed to sync Hikvision users:', error);
    }
  ...
  async getDeviceInfo(): Promise<{ model: string; firmware: string; serialNumber: string; totalUsers: number }> {
    ...
    return { model: 'Hikvision', firmware: 'Unknown', serialNumber: 'Unknown', totalUsers: 0 };
  }
